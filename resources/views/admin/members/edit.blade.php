...
@section('content')
    ...
    <form action="{{ route('members.update', $user->id) }}" method="POST" class="bg-white p-6 shadow rounded space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="nowa" value="{{ old('nowa', $user->nowa) }}"
                   class="w-full border rounded px-3 py-2">
        </div>
...
<select name="role" id="role" 
    ...
    <option value="member" {{ old('role', $user->roles) == 'member' ? 'selected' : '' }}>Member</option>
</select>
        </div>
