...
<div class="p-6">
    ...
    <form action="{{ route('rekening.update', $rekening) }}" method="POST" class="space-y-4 bg-white p-6 shadow rounded-lg">
        @csrf @method('PUT')

        <div>
            <label class="block font-medium">Nama Bank / E-Wallet</label>
            <input type="text" name="bank_name" value="{{ old('bank_name', $rekening->bank_name) }}" 
                   class="w-full border p-2 rounded" required>
        </div>

        <div>
            <label class="block font-medium">Nomor Rekening / Nomor HP</label>
            <input type="text" name="bank_account" value="{{ old('bank_account', $rekening->bank_account) }}" 
                   class="w-full border p-2 rounded" required>
        </div>

        <div>
            <label class="block font-medium">Nama Pemilik</label>
            <input type="text" name="account_name" value="{{ old('account_name', $rekening->account_name) }}" 
                   class="w-full border p-2 rounded" required>
        </div>
